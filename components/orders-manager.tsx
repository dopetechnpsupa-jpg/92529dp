...
export function OrdersManager() {
  ...
  const [orderToDelete, setOrderToDelete] = useState<{ id: number; orderIdString: string } | null>(null)
  ...
  const handleDeleteOrder = (orderId: number, orderIdString: string) => {
    setOrderToDelete({ id: orderId, orderIdString })
  ...
  const confirmDeleteOrder = async () => {
    ...
    try {
      ...
      // Use client-side service instead of API route
      const { ordersClient } = await import('@/lib/orders-client')
      await ordersClient.deleteOrder(orderToDelete.id)
      ...
      // Show success message
      setSuccess(`Order ${orderToDelete.orderIdString} deleted successfully`)
      ...
      setError(`Failed to delete order: ${error instanceof Error ? error.message : 'Unknown error'}`)
    } finally {
      setUpdatingStatus(false)
    }
  }
          ...
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-6 w-full max-w-md">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400">
                Are you sure you want to delete order <span className="font-semibold text-white">{orderToDelete.orderIdString}</span>?
              </p>
            </div>
            
            <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 mb-6">
              <p className="text-sm text-red-400 font-medium mb-2">This action cannot be undone and will permanently remove:</p>
