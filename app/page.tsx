...
import { OptimizedImageEnhanced } from "@/components/optimized-image-enhanced"

// Dynamic imports for better code splitting
const SlidingCardCarousel = dynamic(() => import("@/components/sliding-card-carousel").then(mod => ({ default: mod.SlidingCardCarousel })), {
  loading: () => <div className="flex items-center justify-center h-64 bg-gradient-to-br from-gray-900 to-black rounded-2xl">
...
const DraggableMarquee = dynamic(() => import("@/components/draggable-marquee").then(mod => ({ default: mod.DraggableMarquee })), {
  loading: () => <div className="h-32 bg-gray-800 rounded-xl animate-pulse"></div>,
  ssr: false
})

const DopeDailyShowcase = dynamic(() => import("@/components/dope-daily-showcase").then(mod => ({ default: mod.DopeDailyShowcase })), {
  loading: () => <div className="h-48 bg-gray-800 rounded-xl animate-pulse"></div>,
  ssr: false
})
...
const SupabaseCheckout = dynamic(() => import("@/components/supabase-checkout"), {
  loading: () => <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
    <div className="text-center">
      <div className="w-12 h-12 border-4 border-[#F7DD0F] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
      <p className="text-[#F7DD0F] font-semibold">Loading Checkout...</p>
    </div>
  </div>,
  ssr: false
})

const CartItemEditor = dynamic(() => import("@/components/cart-item-editor").then(mod => ({ default: mod.CartItemEditor })), {
  loading: () => <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
    <div className="text-center">
      <div className="w-12 h-12 border-4 border-[#F7DD0F] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
      <p className="text-[#F7DD0F] font-semibold">Loading Editor...</p>
    </div>
  </div>,
  ssr: false
})
        ...
        if (adminCategories) {
          try {
            ...
            const result = parsed.map((cat: any) => {
              ...
              // Ensure cat has required properties
              if (!cat.id || !cat.name) {
                return {
                  id: cat.id || 'unknown',
...
export default function DopeTechEcommerce() {
  ...
  const handleProductNavigation = (productId: string, productName?: string) => {
    ...
    if (productName) {
      // Use the product name if provided
      const slug = generateSlug(productName)
      const seoUrl = `/product/${productId}-${slug}`
      router.push(seoUrl)
    } else {
      // Fallback: try to find product in the products array
      ...
      if (product) {
        ...
        // Final fallback: use ID-only URL
              ...
              Array.from({ length: 12 }).map((_, index) => (
               ...
               <div key={`${product.id}-${selectedCategory}`} data-product-id={product.id} className="group animate-fade-in-up hover-lift product-card-fluid scroll-animate h-full transition-all duration-300 ease-in-out" style={{ animationDelay: `${index * 0.05}s` }}>
                 <div 
                   ...
                   onClick={() => handleProductNavigation(product.id.toString(), product.name)}
                 >
            ...
            <div className="w-full h-40 sm:h-48 md:h-56 lg:h-64 xl:h-72 2xl:h-80 relative overflow-hidden rounded-2xl">
              {dopePicks.length > 0 ? (
                ...
                <div 
                  ...
                  onClick={() => {
                    if (dopePicks[0]) {
                      handleProductNavigation(dopePicks[0].id.toString(), dopePicks[0].name)
                    }
                  }}
                >
