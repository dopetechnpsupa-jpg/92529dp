...
import { ColorFeatureCombination } from '@/components/color-feature-combination'
...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        ...
        // Always set checking auth to false after the check is complete
        setIsCheckingAuth(false)
      }
    } else {
      // If window is not available (SSR), just set checking to false
      setIsCheckingAuth(false)
    }
  }, [])
  ...
  // Color-feature combinations state
  const [colorFeatureCombinations, setColorFeatureCombinations] = useState<Array<{
    id: string
    color: string
    feature: string
    price: number
  ...
  // Function to calculate discount percentage from original price and discounted price
  ...
  const handleAddProduct = async () => {
    try {
      ...
      const discountedPrice = parseFloat(formData.discounted_price || formData.price)
      const calculatedDiscount = calculateDiscountPercentage(originalPrice, discountedPrice)
      ...
      const newProduct = await addProduct({
        ...
        color_feature_combinations: colorFeatureCombinations, // Add combination pricing
  ...
  const handleEditProduct = async () => {
    ...
    try {
      console.log('🔄 Starting product edit for ID:', editingProduct.id);
      console.log('📝 Form data:', formData);
      console.log('🎨 Color-feature combinations:', colorFeatureCombinations);
  ...
  const handleEditClick = async (product: Product) => {
    ...
    // Use existing color_feature_combinations if available, otherwise parse from features
    let colorFeatureCombos: Array<{ id: string; color: string; feature: string; price: number; original_price?: number; discount?: number }> = []
    let regularFeatures: string[] = []
    
    if (product.color_feature_combinations && product.color_feature_combinations.length > 0) {
      // Use the new combination pricing data
      colorFeatureCombos = product.color_feature_combinations.map(combo => ({
        id: combo.id,
        color: combo.color,
      ...
      existingFeatures.forEach(feature => {
        if (feature.includes(' + ')) {
          const [color, featureName] = feature.split(' + ')
          colorFeatureCombos.push({
            id: Math.random().toString(36).substr(2, 9),
            color: color.trim(),
            feature: featureName.trim(),
            price: product.price, // Use product price as fallback
    ...
    // Set the parsed color-feature combinations
    setColorFeatureCombinations(colorFeatureCombos)
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-8 w-full max-w-md">
          <div className="text-center">
            <div className="w-16 h-16 bg-[#F7DD0F]/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <Package className="w-8 h-8 text-[#F7DD0F] animate-pulse" />
            </div>
            <h1 className="text-2xl font-bold text-white mb-2">DopeTech Admin</h1>
            <p className="text-gray-400">Checking authentication...</p>
            <div className="mt-4">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#F7DD0F] mx-auto"></div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y min-h-[120px] font-mono text-sm leading-relaxed"
                ...
                <div>
                  ...
                  {formData.original_price && formData.discounted_price && (
                    <div className="mt-2 text-sm text-gray-400">
                      Discount: {calculateDiscountPercentage(parseFloat(formData.original_price), parseFloat(formData.discounted_price))}%
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Color-Feature Combination System */}
              <div className="col-span-2">
                <ColorFeatureCombination
                  combinations={colorFeatureCombinations}
                  onCombinationsChange={setColorFeatureCombinations}
                />
              </div>
              ...
              <div className="col-span-2">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Enter additional features separated by commas (e.g., Wireless, RGB, Bluetooth)"
                  rows={2}
                />
                <p className="text-xs text-gray-400 mt-1">
                  These will be added alongside your color-feature combinations
                </p>
              </div>
