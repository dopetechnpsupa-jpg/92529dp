...
const CACHE_TTL = {
  ...
  PRODUCT_IMAGES: 10 * 60 * 1000, // 10 minutes
  CATEGORIES: 15 * 60 * 1000, // 15 minutes
  DOPE_PICKS: 2 * 60 * 1000, // 2 minutes
};
...
// Color-Feature Combination with pricing
export interface ColorFeatureCombination {
  id: string;
  color: string;
  feature: string;
  price: number;
...
export interface Product {
  ...
  color_feature_combinations?: ColorFeatureCombination[]; // New field for combination pricing
  in_stock: boolean;
  discount: number;
...
// Fetch products from Supabase with local fallback and caching
export async function getProducts(): Promise<Product[]> {
  ...
  try {
    ...
    // Cache the result
    setCachedData(cacheKey, products, CACHE_TTL.PRODUCTS);
    ...
    setCachedData(cacheKey, fallbackProducts, CACHE_TTL.PRODUCTS);
...
export async function addProduct(productData: Omit<Product, 'id' | 'rating' | 'reviews'> & { features?: string[], color?: string, color_feature_combinations?: ColorFeatureCombination[] }): Promise<Product | null> {
  try {
    // Prepare insert data
    ...
    // Add color_feature_combinations if provided
    if (productData.color_feature_combinations !== undefined) {
      insertData.color_feature_combinations = productData.color_feature_combinations;
    }
    ...
    if (error) {
      ...
      // If the error is related to color_feature_combinations column not existing,
      // try adding without it
      if (error.message && error.message.includes('color_feature_combinations')) {
        console.log('🔄 Retrying add product without color_feature_combinations field...');
        ...
        if (retryError) {
          console.error('Error adding product (retry):', retryError);
          return null;
        }

        console.log('✅ Product added successfully (without combinations):', retryData);
        return retryData as unknown as Product;
      }
...
export async function updateProduct(productId: number, productData: Partial<Omit<Product, 'id' | 'rating' | 'reviews'>> & { features?: string[], color?: string, color_feature_combinations?: ColorFeatureCombination[] }): Promise<Product | null> {
  try {
    console.log(`🔄 Updating product ${productId} with data:`, productData);
    
    // Prepare update data, excluding color_feature_combinations initially
    const updateData: any = {
      ...
      hidden_on_home: productData.hidden_on_home || false,
      updated_at: new Date().toISOString()
    };

    // Try to update with color_feature_combinations first
    if (productData.color_feature_combinations !== undefined) {
      updateData.color_feature_combinations = productData.color_feature_combinations;
    }
    ...
    if (error) {
      ...
      // If the error is related to color_feature_combinations column not existing,
      // try updating without it
      if (error.message && error.message.includes('color_feature_combinations')) {
        console.log('🔄 Retrying update without color_feature_combinations field...');
        ...
        console.log(`✅ Product ${productId} updated successfully (without combinations):`, retryData);
        return retryData as unknown as Product;
      }
